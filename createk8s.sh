...
shell_log "レジストリのリソース・グループを作成します ["  "$command" "]"
result=eval $command
ret=$?
...
shell_log "コンテナを登録するためのレジストリを作成します ["　"$command" "]"
result=eval $command
ret=$?
...
# --　AKSやACRを動的に操作するための Service Principalを作成。 Service Principal の Password の作成
...
# -- 確認
shell_log "以下の情報を使って、AKSは、ACRからコンテナを取得します。"
...
shell_Log "Budiling practice enviroment complete. start=" $START_TIME ",end=" $END_TIME
